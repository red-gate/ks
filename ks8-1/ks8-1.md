...
1. Create the Kubernetes deployment definition that will be used to spin up a PostgreSQL pod
    ...
    **Note**: In this episode we are storing passwords in our deployment yaml file `ks.database.deployment.yaml`. This is not ideal, we should use **secrets** instead as we will see in a future episode.
