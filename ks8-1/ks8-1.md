...
The objective here is to remove the in-memory todo list store and instead use a database. We will be using PostgreSQL for our database.
...
However, we'll discover that this is not ideal as the data in PostgreSQL is not persisted if our database pod goes down for any reason. In the next episodes, we'll see how to fix this problem.
...
1. start minikube and build docker images
    ...
    ➜ docker build -f ./database/Dockerfile -t ks8webimage .
    ```
